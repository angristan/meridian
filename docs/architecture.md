...
The Durable Object reconciles an R2 blob catalog when storage is inspected. Upload and provisional commit claims protect in-flight blobs from orphan cleanup. Cleanup installs a SQL deletion fence before deleting from R2, while commits reserve each blob before awaiting R2. A revision cannot commit until every referenced blob is confirmed in R2 and its claim is rechecked in the operation transaction. Attachment uploads and downloads use at most four concurrent chunk transfers. A started revision upload still reaches its commit boundary before pause.
